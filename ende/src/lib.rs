...
use crate::io::{BorrowRead, Read, SizeLimit, SizeWrite, Write};
...
impl<T: Write> Encoder<'_, T> {
	...
	///
	/// let mut encoder = Encoder::new((), Context::new());
	/// encoder.write_str("Hello, world!".chars()).unwrap();
	/// ```
	pub fn write_str<S, I>(&mut self, string: S) -> EncodingResult<()>
	where S: IntoIterator<Item = char, IntoIter = I>,
	      I: Iterator<Item = char> + Clone
	{
		let chars = string.into_iter();
		let encoding = self.ctxt.settings.string_repr.str_encoding;
		match self.ctxt.settings.string_repr.len_encoding {
			StrLenEncoding::LenPrefixed => {
				...
				let mut sz_encoder = Encoder::new(SizeWrite::new(), self.ctxt);
				for ch in chars.clone() {
					sz_encoder.write_char(ch)?;
				}
				let size = sz_encoder.stream.size();
			...
			StrLenEncoding::NullTerminated => {
				...
				// Currently the encoding with the biggest code unit is utf-32 which uses 4 bytes.
				// This number needs to be increased if an encoding with bigger code units
				// is introduced
				// Alternatively, this could be converted into a for loop.
				let zero = [0u8; 4];
				self.write_raw_bytes(&zero[..encoding.bytes()])?;
			}
		}
		Ok(())
	}
...
macro_rules! make_borrow_slice_fn {
    ($name:ident -> $ty:ty) => {
	    ...
	    pub fn $name(&mut self, length: usize, num_encoding: NumEncoding, endianness: Endianness) -> EncodingResult<&'data [$ty]>
		{
			// Assert the num encoding is borrowable
			if !num_encoding.borrowable() {
				return Err(EncodingError::BorrowError(BorrowError::NonBorrowableNumEncoding {
					num_encoding,
				}))
			}
			...
			if endianness != Endianness::native() {
				return Err(EncodingError::BorrowError(BorrowError::EndiannessMismatch {
					found: endianness,
...
impl<'data, T: BorrowRead<'data>> Encoder<'_, T> {
	...
	/// let encoder = Encoder::new(&slice[..], Context::new());
	///
...
/// A binary data structure specification which can be **decoded** from its binary representation
/// into a type which contains borrowed data.
