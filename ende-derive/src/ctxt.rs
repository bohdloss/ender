...
use syn::{Attribute, Data, DeriveInput, Error, Expr, Fields, Generics, Index, parse_quote, Type};
...
impl Ctxt {
	pub fn parse_from(input: &DeriveInput, target: Target) ->  syn::Result<Self> {
		...
		let item_type = match &input.data {
			...
			Data::Union(data) => return Err(Error::new(
...
pub fn extract_fields_and_flavor(fields: &Fields, target: Target) -> syn::Result<(Flavor, Vec<Field>)> {
	match fields {
		Fields::Named(fields) => {
			Ok((
				Flavor::Struct,
				process_fields(fields.named.iter(), target)?
			))
		}
...
pub fn process_fields<'a>(syn_fields: impl Iterator<Item = &'a syn::Field>, target: Target) -> syn::Result<Vec<Field>> {
	let mut idx: usize = 0;
	...
	for syn_field in syn_fields {
		...
		let (accessor, name) = if let Some(name) = &syn_field.ident {
			...
			(Index { index: idx as u32, span: syn_field.span() }.to_token_stream(), format_ident!("m{}", idx))
		};
