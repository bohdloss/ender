...
use quote::ToTokens;
use syn::{GenericArgument, Lifetime, Path, PathArguments, Type, TypeParamBound, TypeReference, TypeSlice};

use crate::ctxt::Field;

/// Searches through the fields to find all the lifetime bounds
/// required by a `BorrowDecode` implementation
pub fn discover_lifetime_bounds(fields: &mut [Field]) -> syn::Result<Vec<Lifetime>> {
...
pub fn discover_field_lifetime_bounds(field: &Field) -> syn::Result<Vec<Lifetime>> {
	if let Some(borrow) = &field.flags.borrow {
		if borrow.len() > 0 {
			...
			recursive_type_lifetime_discover(&field.ty, &mut lifetimes)?;

			Ok(lifetimes)
		}
	} else {
		// No borrow flag was not declared, but maybe we can infer the lifetime for simple types
		match &field.ty {
			Type::Reference(TypeReference { lifetime: Some(lif), elem, .. }) => {
				match elem.deref() {
...
fn recursive_type_lifetime_discover(ty: &Type, vec: &mut Vec<Lifetime>) -> syn::Result<()> {
	match ty {
		Type::Array(array) => {
			recursive_type_lifetime_discover(&array.elem, vec)
		}
		Type::BareFn(_) => { Ok(()) }
		Type::Group(group) => {
			recursive_type_lifetime_discover(&group.elem, vec)
		}
		...
		Type::Paren(paren) => {
			recursive_type_lifetime_discover(&paren.elem, vec)
		}
		Type::Path(path) => {
		...
		Type::Ptr(ptr) => {
			recursive_type_lifetime_discover(&ptr.elem, vec)
		}
		...
		Type::Slice(slice) => {
			recursive_type_lifetime_discover(&slice.elem, vec)
		}
...
fn recursive_type_param_bound_lifetime_discover(bound: &TypeParamBound, vec: &mut Vec<Lifetime>) -> syn::Result<()> {
	match bound {
...
fn recursive_generics_lifetime_discover(arg: &GenericArgument, vec: &mut Vec<Lifetime>) -> syn::Result<()> {
	match arg {
